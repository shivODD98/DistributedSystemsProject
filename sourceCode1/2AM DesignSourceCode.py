...
#python Client/client.pyfrom threading import Lock
import threading
from datetime import datetime
import sys
...
class Peer:
    ...
    def resendAck(self):
        ...
        if self.retrys > 3:
            ...
            print(f"Canceling ACK for {self.peer}")
            return
...
class GroupManager:
    ...
    def received_ack(self, peerAddress):
        print(f'received ack from {peerAddress}')
        for i in range(len(self.__list)):
            print(self.__list[i].ackTimer)  
            if self.__list[i].peer == peerAddress:
        ...
        while self.isAlive:
            if self.isAlive:
                ...
                if not data:
                    ...
                    print(f'recieved snip message from:{addr}. Sending ack {snipData[0]}')
